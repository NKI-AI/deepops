...
if argv[1] == "update_order":
    ...
    for k in updateItems:
        updateItems[k] = [
            i for n, i in enumerate(updateItems[k]) if i not in updateItems[k][n + 1 :]
        ]
...
if argv[1] == "parse_update_json":
    ...
    fw_update_json = {"Error": True, "State": "Unknown", "Action": "Check Output Log"}
    ...
    for line in fw_update:
        try:
            ...
            if (
                "FirmwareLoadAction" in json.loads(line).keys()
            ):  # Detects if chassis-level power cycle is required
                fw_update_json = json.loads(line)
            if (
                "Reboot required" in lineJson["Message"]
            ):  # Detects if host-level reboot is required
...
if argv[1] == "parse_versioning":
    ...
    manifest_json = {"ErrorWritingVersioning": True}
    ...
    try:
        ...
        print(json.dumps(manifest_json, sort_keys=True, indent=4))
